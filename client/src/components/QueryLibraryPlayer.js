...
export default function LibraryPlayer() {
  ...
  const { like, unlike, isLiked, checkIfLiked } = useContext(LibraryContext)
 ...
 useEffect(() => {
  ...
  const libraryDevice = localStorage.getItem('device')

  useEffect(() => {
    if(playingTrack && Object.keys(playingTrack).length > 0 && user) {
    checkIfLiked(libraryTrack.uri);
    }
  }, [libraryTrack])
  ...
  return (
    ...
    <footer className="now-playing-bar">
      <div className="player-wrapper">
        ...
        { 
          libraryTrack && 
          Object.keys(libraryTrack).length > 0 && 
          <div className="now-playing-widget">
            <div className="now-playing-icon">
              <img src={libraryTrack.album.images[2].url} style={{ height: '56px', width: '56px' }} />
            </div>
            <div className="now-playing-text">
              <div className="now-playing-title">
                <span>{libraryTrack.name}</span>
              </div>
              <div className="now-playing-artist">
                <span>{libraryTrack.artists.map((artist, i, artists) => artists.length - 1 === i ? artist.name : artist.name + ', ' )}</span>
              </div>
            </div>
            <>
            {
             ...
             <></>
              ...
              onClick={() => onLike(libraryTrack.name, playingTrack.uri, 
              libraryTrack.artists, libraryTrack.album.images[2].url, 
              libraryTrack.album.name, libraryTrack.duration_ms, libraryTrack.id)
            }  
              ...
              :
              <button className="like-btn fill" onClick={() => onRemoveLike(libraryTrack.id)} ref={greenHeartRef}>
          ...
          <div className="playback-controls">
            <div className="playback-widget">
              <div className="controls-left">
                <button className="previous-btn" onClick={() => handleSkipPrev(libraryDevice)}>
              ...
              <div className="controls-right">
                <button className="skip-btn" onClick={() => handleSkipNext(libraryDevice)}>
                <svg role="img" height="16" width="16" viewBox="0 0 16 16" >
            ...
            {/* <div className="playback-bar">
              <div className="playback-position">0:15</div>
              <label className="hidden-slider">
                Change progress
                <input type="range" min="0" max="196" step="5" value="2" />
              </label>
              ...
              <div className="playback-duration">3:27</div>
            </div>  */}
          </div>
        </div>
